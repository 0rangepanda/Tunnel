...
* map the incoming circuit ID to its outgoing counterpart, then forward the
* message.
**************************************************************************/
int RouterClass::enc_recvFake(CtlmsgClass *recv_ctlmsg, __u16 inc_port) {
        ...
        // has not seen before
        {
                ...
                strncpy(keyMap[incId], recv_ctlmsg->getPayload(),
                        recv_ctlmsg->getPayloadLen());
                LOG(logfd, "fake-diffie-hellman, new circuit incoming: 0x%x, key:%s\n",
                    incId,
                    packet_str(recv_ctlmsg->getPayload(), recv_ctlmsg->getPayloadLen()));
        } else {
                ...
                aes->decrypt((unsigned char *)recv_ctlmsg->getPayload(),
                             recv_ctlmsg->getPayloadLen(), (unsigned char **)&tmp,
                             &tmp_len);
                ...
                LOG(logfd,
                    "fake-diffie-hellman, forwarding,  circuit incoming: 0x%x, key:%s\n",
                    incId,
                    packet_str(recv_ctlmsg->getPayload(), recv_ctlmsg->getPayloadLen()));
        }
...
int RouterClass::enc_sendRlyret(Packet *p, __u16 out_port, int outId) {
        ...
        int tmp_len;
        aes->set_encrypt_key(keyMap[outId]);
        aes->encrpyt((unsigned char *)p->getPacket(), p->getPacketLen(),
...
/**************************************************************************
* To return a packet to the proxy, the router will use a
* relay-return-encrypted-data control message. The relay-return-encrypted-data
* message will have type 0x64, with two bytes for the circuit ID and the
* appended packet contents. It should zero the destination IP address, just as
* the OP zeroed the source IP address.
**************************************************************************/
int RouterClass::handle_ICMPFromRaw_6(Packet *p) {
