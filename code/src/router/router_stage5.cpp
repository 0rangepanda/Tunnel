...
**************************************************************************/
int RouterClass::readFromProxy_5()
{
...
* will relay the message down the circuit, to the next-hop router it saved before.
* When doing so, it must map the incoming circuit ID (0xIDi) to the outgoing
* circuit ID (0xIDo), and pass on NEXT-NAME for the new next hop.
**************************************************************************/
...
int RouterClass::recvExtDone(CtlmsgClass* recv_ctlmsg, __u16 inc_port)
{
        ...
        LOG(logfd, "forwarding extend-done circuit, incoming: 0x%x, outgoing: 0x%x at %d\n",
            incId, outId, outport);
...
*
* When the packet arrives at the end of the circuit, as indicated by a NEXT-NAME of 0xffff,
* the router should send the packet out to the Internet. To do so,
* change the source IP address [of the encapsulated packet] to that of the router.
* Then write the packet out its raw ICMP socket.
*
...
int RouterClass::recvRlydata(CtlmsgClass* recv_ctlmsg, __u16 inc_port)
{
        ...
        {
                /* forward */
                LOG(logfd, "relay packet, circuit incoming: 0x%x, outgoing: 0x%x, incoming src: %s, outgoing src: %s, dst: %s\n",
